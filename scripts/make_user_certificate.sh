...
EXPIRY="always:forever"
# options may be followed by one colon to indicate they have a required argument
SHORT_OPTS="uhdxep:U:M:D:t:f:n:W:A:C:V:i:z:O:"
LONG_OPTS="user,host,debug,xdebug,echo,priority:,user-name:,user-home:,domain-list:,cipher:,tarfile:,outfile:,principals:,workdir:,authority:,ca-dir:,expiration:,key-id:,serial-number:command:"
...
do
  case $1 in
    ...
    -O|--command)        FORCE_COMMAND="-O force-command=\'$2\'"; shift;;
    (--)                 shift; break;;
...
$ECHO ssh-keygen -t "${CIPHER}" -f "$WORKDIR/.ssh/$OUTFILE" -N "$PASSPHRASE"
# 2. Sign the new key pair.
echo $0: Signing public key ${OUTFILE}.pub
if [ -z "$PRINCIPALS" ] ; then
    PRINCIPALS="$USER_NAME"
fi
$ECHO ssh-keygen -s "${CA_PATH}" -V "$EXPIRY" -I "${KEY_ID}" -z "${SERIAL_NUMBER}" \
	-n "$PRINCIPALS" "$WORKDIR/.ssh/${OUTFILE}.pub"
# 3. Create a stanza for $WORKDIR/.ssh_config.d.
cat > $WORKDIR/.ssh/ssh_config.d/$PRIORITY.${OUTFILE} << EOF
Host $DOMAIN_LIST
        Hostname %h
        User $USER_NAME
...
$ECHO tar --directory "$WORKDIR" --create --gzip --file ${OUTFILE}.tgz .ssh
